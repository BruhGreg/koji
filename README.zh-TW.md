...
koji 讓你的 AI 代理擁有跨工作階段的記憶：經驗教訓、專案狀態交接，以及自動歸檔的工作階段日誌。八個技能,一次安裝。純 bash + markdown,無需建置步驟。
...
**焦點過濾的 kick-off 上下文。** `/kick-off` 不會把所有東西都倒出來——它會根據你的 kick-off 參數、上次工作階段的註記、未完成的 TODO 項目,以及前 3 名最近性基準,載入 `lessons.md` 的焦點過濾子集。可用 `YYYY-MM-DD — [Agent] — [領域1,領域2] — …` 標記項目以加強匹配。
...
**Duet 工作流程。** 不會卡住使用者的跨模型代理協作。`/duet-plan` 執行多輪 Claude↔codex 對話直到共識,鎖定計畫。`/duet-impl` 依關卡逐步走過計畫,每關 codex 審查。`/duet-review` 進行雙審查者對抗式檢查,意見分歧時交叉審查。三個技能皆以背景任務執行審查者——進行中你可以繼續工作。
