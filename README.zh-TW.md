...
`/wrap` 讓 `## Load on Kick-Off` 與專案走向同步。三個 pass 匯入單一整合提案：

- **新增**——已標籤文件的 `covers:` 與本工作階段觸碰的檔案重疊；以及主題與下一個工作階段任務（依據 `TODO.md`、「Notes for Next Session」、啟動提示、對話上下文）相符的已標籤文件。
- **移除**——目前已載入但已不再相關的文件。保守的守門條件會跳過移除：本次 wrap 才剛新增的、狀態為 `stale`/`orphan`（可能正是要修的）、或覆蓋下一個工作階段路徑的。

互動模式只問一次：全部套用 / 僅新增 / 僅移除 / 逐項挑選 / 跳過。自動模式（無法觸發提示）下，新增與具決定性的移除（覆蓋路徑超過閾值未被觸碰）會自動套用，僅憑判斷的移除則僅列為建議；最後印出一行說明變更內容與如何回復（`git restore`）。清單不會跨工作階段無上限地膨脹。
...
1. **教訓** — 預設動作為**跳過**。候選條目必須通過三道閘門（5 分鐘內可重新發現 / 框架常識 / 已造成實際時間損失）並通過「使用者自己會寫下這條嗎？」測試後，才能追加至 `lessons.md`。大多數工作階段新增零條——這就是成功路徑。格式：`YYYY-MM-DD — [Agent] — 出了什麼問題 → 預防規則`。
