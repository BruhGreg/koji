...
**Codex dispatch.** Every codex call koji makes runs under a deadline and inside one wrapper, on every machine — including a stock macOS with no `coreutils`, where the old path silently ran uncapped. Nothing a review spawns outlives it. The failure modes are separated so none of them can read as a clean review: a session that is *itself* running under codex stops the run outright (the outside voice cannot be the same model, and substituting a Claude reviewer there would be Reviewer A grading itself); a dispatch block that loses a variable or builds an empty prompt is reported as a broken block and never quietly degraded into a substitute reviewer; and a genuine environment failure — a full temp dir, a hung CLI — degrades the way a reviewer failure always has, with the banner that says so. koji also warns on the codex CLI versions with known stdin deadlocks, since stdin is how it feeds every prompt.
