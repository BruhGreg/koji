...
**Walk-away sessions.** `/duet-plan`, `/duet-impl`, and `/triangulate` keep the machine awake (`caffeinate` / `systemd-inhibit`) while their background AI dispatches run, then release it at the end — so you can start a long run and step away. Opt-in to those skills only (never plain `/kick-off`); reference-counted so nested runs share one keep-awake, and ownership-safe — a keep-awake you started yourself is never touched.
